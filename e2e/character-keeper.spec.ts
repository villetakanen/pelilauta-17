...
test.describe('Character Keeper', () => {
  ...
  test('can view character in keeper', async ({ page }) => {
    ...
    const selectorOptions = await page.locator('select option').allTextContents();
    ...
    const selectedValue = await page.locator('.select-wrapper select, div select').first().inputValue();
    ...
    if (errorCards > 0) {
      const errorText = await page.locator('cn-card[noun="error"]').first().textContent();
    ...
    if (infoCards > 0) {
      const infoText = await page.locator('cn-card[noun="info"]').first().textContent();
      console.log('Info message on page:', infoText);
      
      // Get the title and description separately
      const titleAttr = await page.locator('cn-card[noun="info"]').first().getAttribute('title');
      const descAttr = await page.locator('cn-card[noun="info"]').first().getAttribute('description');
    ...
    if (selectorOptions.length > 1) {
      const sheetToSelect = selectorOptions.find(option => option !== 'Valitse hahmolomake' && option.trim() !== '');
      if (sheetToSelect) {
        console.log('Selecting sheet:', sheetToSelect);
        await page.locator('.select-wrapper select, div select').first().selectOption({ label: sheetToSelect });
        
        // Wait for the selection to be processed
        await page.waitForTimeout(3000);
        
        // Verify selection
        const newSelectedValue = await page.locator('.select-wrapper select, div select').first().inputValue();
        ...
        const infoCardsAfter = await page.locator('cn-card[noun="info"]').count();
        console.log('Info cards after sheet selection:', infoCardsAfter);
        if (infoCardsAfter > 0) {
          const infoTextAfter = await page.locator('cn-card[noun="info"]').first().textContent();
          const titleAfter = await page.locator('cn-card[noun="info"]').first().getAttribute('title');
          const descAfter = await page.locator('cn-card[noun="info"]').first().getAttribute('description');
    ...
    const loaders = await page.locator('cn-loader, .loading, [class*="loading"]').count();
    console.log('Loading indicators found:', loaders);

    // Check if there are any character cards after sheet selection
    const characterCards = await page.locator('[data-testid="character-card"], .character-card, cn-card:not([noun])').count();
    ...
    const characterNameElements = await page.locator('text=E2E Keeper Test Character').count();

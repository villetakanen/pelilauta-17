...
async function cleanupTestThreads() {
  ...
  for (const threadDoc of testThreadQuery.docs) {
    ...
    const isMarkedForCleanup = data.markdownContent?.includes('automatically cleaned up after the test runs');
  ...
  for (const thread of threadsToDelete) {
    console.log(`  - ${thread.title} (${thread.key}) created at ${thread.createdAt.toISOString()}`);
  }
...
async function updateChannelCounts() {
  ...
  try {
    ...
    for (const channel of channelsArray) {
      ...
      console.log(`  Channel "${channel.name}" (${channel.slug}): ${channel.threadCount} threads`);
    }

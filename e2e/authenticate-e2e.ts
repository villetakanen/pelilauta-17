...
export async function authenticate(page, useNewUserAccount = false) {
  ...
  const errorElements = await page.locator('cn-snackbar, .error, [role="alert"]').all();
  ...
  const isEmailFieldStillVisible = await page.getByLabel('Email').isVisible().catch(() => false);
  if (isEmailFieldStillVisible) {
    ...
    const isButtonLoading = await page.getByRole('button', { name: 'Login' }).getAttribute('disabled');

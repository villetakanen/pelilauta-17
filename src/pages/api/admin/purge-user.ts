...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    logDebug('api/admin/purge-user', `Purging user ${userToPurgeUid} by admin ${adminUid}`);

    // 1. Clear custom claims
    try {
      await serverAuth.getUser(userToPurgeUid);
      await serverAuth.setCustomUserClaims(userToPurgeUid, {});
      logDebug('api/admin/purge-user', `Cleared custom claims for ${userToPurgeUid}`);
    } catch (error: any) {
      if (error.code === 'auth/user-not-found') {
        logDebug('api/admin/purge-user', `User ${userToPurgeUid} not found in Auth, skipping claim removal.`);
      } else {
    ...
    const accountRef = serverDB.collection(ACCOUNTS_COLLECTION_NAME).doc(userToPurgeUid);
    await accountRef.delete();
    logDebug('api/admin/purge-user', `Deleted account document for ${userToPurgeUid}`);

    // 3. Delete profile document
    const profileRef = serverDB.collection(PROFILES_COLLECTION_NAME).doc(userToPurgeUid);
    await profileRef.delete();
    logDebug('api/admin/purge-user', `Deleted profile document for ${userToPurgeUid}`);

...
export async function handlePageUpdate(
  ...
  try {
    ...
    if (!purger.isConfigured()) {
      logWarn('handlePageUpdate', 'Netlify cache purging not configured, skipping cache purge', {
        siteKey,
        pageKey,
    ...
    logError('handlePageUpdate', 'Failed to purge cache (page update will continue)', {
      siteKey,
      pageKey,
 ...
 * @returns Promise<void>
 */
export async function handleSiteUpdate(siteKey: string, site: Site): Promise<void> {
  try {
    const purger = new NetlifyCachePurger();
    
    if (!purger.isConfigured()) {
      logWarn('handleSiteUpdate', 'Netlify cache purging not configured, skipping cache purge', {
        siteKey,
        config: purger.getConfigStatus(),
      });
      return;
    ...
    if (result.success) {
      logDebug('handleSiteUpdate', 'Site-wide cache purge completed successfully', {
        siteKey,
        tags: result.tags,
      });
    } else {
...
export async function handleBulkPageUpdate(
  ...
  try {
    ...
    if (!purger.isConfigured()) {
      logWarn('handleBulkPageUpdate', 'Netlify cache purging not configured, skipping cache purge', {
        siteKey,
    ...
    if (result.success) {
      logDebug('handleBulkPageUpdate', 'Bulk cache purge completed successfully', {
        siteKey,
        pageCount: pageKeys.length,
...
export function isCachePurgingConfigured(): boolean {
  try {
    ...
    logError('isCachePurgingConfigured', 'Failed to check cache purging configuration', error);
    return false;
  }
}
